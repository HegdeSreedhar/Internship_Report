...
\addchap*{Abstract}
With the increased data transfers within a vehicle between different ECUs and the need for higher bandwidth, Automotive Ethernet has gained significant traction and is poised to serve as the vehicle domain's communication backbone. BMW designed the SOME/IP communication control protocol for automotive use cases to efficiently manage network traffic in order to meet these needs. A demonstrator consisting of multiple devices communicating over Ethernet using SOME/IP has been set up to understand the use cases of this technology. The applications are built in a POSIX-based environment and adhere to a service-oriented architecture. This report goes into great detail about the technology and its practical applications. Along with it, the experiment's results have been documented, which can be used as the foundation for future research into this technology.
